...
public class GetCategoryTask extends AsyncTask<String, Void, Void> {
    ...
    protected final Context tContext;

    public static final String TAG = GetCategoryTask.class.getSimpleName();
    ...
    private ContentValues addCategory(String apiId, String urlImg, String categoryName){
        ...
        Cursor categoryCursor = tContext.getContentResolver().query(
    ...
    private void getCategoryFromJson(String jsonCategoryStr) throws JSONException{
        ...
        try{
            ...
            if (cVVector.size() > 0 ) {
                ...
                inserted = tContext.getContentResolver().bulkInsert(AppDataContract.CategoryEntry.CONTENT_URI, cvArray);
            }
    ...
    private void debugContentValues(ContentValues content){
        Log.d(TAG, "apiId: " + content.getAsString(AppDataContract.CategoryEntry.COLUMN_CATEGORY_API_ID) +
        " category_name: " + content.getAsString(AppDataContract.CategoryEntry.COLUMN_CATEGORY_NAME));
    }

    private void debugVector(Vector<ContentValues> contentValueVector){
        for (ContentValues contentValues : contentValueVector) {
            Log.d(TAG, "Debug: api_id: " + contentValues.getAsString(AppDataContract.CategoryEntry.COLUMN_CATEGORY_API_ID) +
            ", name: " + contentValues.getAsString(AppDataContract.CategoryEntry.COLUMN_CATEGORY_NAME));
        }
    }

    public void createSnack(int coordId, String snackMessage){
        Activity act = (Activity)tContext;
        if(coordLayout == null)
            coordLayout = (CoordinatorLayout)act.findViewById(coordId);
        Snackbar
                .make(coordLayout, snackMessage, Snackbar.LENGTH_SHORT)
                .show();
    }
    ...
    protected void onPreExecute() {
        super.onPreExecute();
        createSnack(R.id.coord_layout_main, tContext.getString(R.string.syncing_category_list));
    }
    ...
    protected void onPostExecute(Void aVoid) {
        ...
        createSnack(R.id.coord_layout_main, "Category List Synced");
    }
